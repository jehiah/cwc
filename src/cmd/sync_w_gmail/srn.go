...
var tlcPattern = regexp.MustCompile(`NOTICE\s+OF\s+(?:HEARING|ADJOURNMENT)\s+(?:AND HEARING LOCATION\s+)?(10[0-9]{6})C`)
...
func TLCIDFromSubject(s string) string {
	s = strings.ToUpper(s)
	if v := tlcPattern.FindStringSubmatch(s); len(v) > 2 {
		return v[1]
	}
	return ""
}
